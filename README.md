...
<body>
    ...
    <script>
        ...
        function sendMessage() {
            ...
            setTimeout(() => {
                hideTypingIndicator();
                const quote = getRelevantQuote(message);
                addMessage('', false, quote);
                if (Math.random() > 0.7) {
                    setTimeout(() => {
                        showTypingIndicator();
                        setTimeout(() => {
                            hideTypingIndicator();
                            const followUpQuote = getRelevantQuote(message);
                            addMessage('', false, followUpQuote);
                        }, Math.random() * 1100 + 450);
                    }, Math.random() * 1500 + 800);
                }
            }, Math.random() * 1700 + 900);
        }
        ...
        setTimeout(() => {
            ...
            addMessage('', false, welcomeQuote);
        }, 900);

        // Idle messages
        let idleTimer;
        const idleMessages = [
            "What philosophical question weighs on your mind today?",
            "The pursuit of wisdom begins with a question. What's yours?",
            "Great thinkers throughout history await your inquiry.",
            "Every question is a door to new understanding."
        ];
            ...
            idleTimer = setTimeout(() => {
                if (chatMessages.children.length < 3) {
                    ...
                    setTimeout(() => {
                        hideTypingIndicator();
                        const idleQuote = {
                            quote: idleMessages[Math.floor(Math.random() * idleMessages.length)],
                            philosopher: "Philosopher's Wisdom",
                            category: "engagement"
                        };
                        addMessage('', false, idleQuote);
                    }, 1300);
                }
            }, 15000);
        }
